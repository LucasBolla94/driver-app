-- =====================================================
-- CRIAR TABELA job_offers_uk COM ÍNDICES CORRETOS
-- =====================================================

-- 1. CRIAR TABELA (se não existir)
CREATE TABLE IF NOT EXISTS job_offers_uk (
  ...
  CONSTRAINT job_offers_uk_status_check
...
CREATE INDEX IF NOT EXISTS idx_job_offers_uk_driver_waiting
ON job_offers_uk(driver_uid, status)
WHERE status = 'waiting';

-- Índice para busca por job_id
CREATE INDEX IF NOT EXISTS idx_job_offers_uk_job_id
ON job_offers_uk(job_id);

-- Índice para busca por expires_at (para cron job expirar ofertas)
CREATE INDEX IF NOT EXISTS idx_job_offers_uk_expires_at
ON job_offers_uk(expires_at)
WHERE status = 'waiting';

-- Índice composto para queries complexas
CREATE INDEX IF NOT EXISTS idx_job_offers_uk_driver_job
ON job_offers_uk(driver_uid, job_id, status);

-- 3. HABILITAR ROW LEVEL SECURITY
ALTER TABLE job_offers_uk ENABLE ROW LEVEL SECURITY;

-- 4. REMOVER POLÍTICAS ANTIGAS (se existirem)
DROP POLICY IF EXISTS "Drivers can view their own offers" ON job_offers_uk;
DROP POLICY IF EXISTS "Drivers can update their own offers" ON job_offers_uk;
DROP POLICY IF EXISTS "Service role full access" ON job_offers_uk;
...
COMMENT ON TABLE job_offers_uk IS 'Tabela de ofertas de jobs para drivers';
COMMENT ON COLUMN job_offers_uk.driver_uid IS 'UID do driver que recebeu a oferta';
COMMENT ON COLUMN job_offers_uk.status IS 'Status da oferta: waiting, accepted, rejected, expired';
COMMENT ON COLUMN job_offers_uk.job_id IS 'ID do job da tabela jobs_uk';
COMMENT ON COLUMN job_offers_uk.expires_at IS 'Data/hora de expiração da oferta';
